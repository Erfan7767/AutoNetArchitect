...
from site_agent.lab_authorization import LaboratoryAuthorization
...
class WindowsWorkspace:
    ...
    def __init__(self, root: Path) -> None:
        ...
        self._laboratory_authorization_path = root / "laboratory_authorization.json"
    ...
    def save_laboratory_authorization(self, authorization: LaboratoryAuthorization) -> LaboratoryAuthorization:
        """Persist a written non-production lab authorization atomically without target credentials or secret values."""

        self._root.mkdir(parents=True, exist_ok=True)
        temporary = self._laboratory_authorization_path.with_suffix(".tmp")
        temporary.write_text(json.dumps(authorization.model_dump(mode="json"), indent=2, sort_keys=True), encoding="utf-8")
        temporary.replace(self._laboratory_authorization_path)
        try:
            self._laboratory_authorization_path.chmod(0o600)
        except OSError:
            return authorization
        return authorization

    def load_laboratory_authorization(self) -> LaboratoryAuthorization | None:
        """Return the saved written laboratory authorization, never a production-target authorization."""

        if not self._laboratory_authorization_path.exists():
            return None
        raw: Any = json.loads(self._laboratory_authorization_path.read_text(encoding="utf-8"))
        if not isinstance(raw, dict):
            raise ValueError("The local laboratory authorization file is invalid.")
        return LaboratoryAuthorization.model_validate(raw)
