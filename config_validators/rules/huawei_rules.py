...
KNOWN_COMMANDS = {
    ...
    "rule_21": CommandRule(r"^sysname\\s+\\S+$", valid_modes=('global',), description='VRP system name'),
    "rule_22": CommandRule(r"^ospf\\s+\\d+(?:\\s+router-id\\s+\\S+)?$", valid_modes=('global',), description='VRP OSPF'),
    "rule_23": CommandRule(r"^bgp\\s+\\d+$", valid_modes=('global',), description='VRP BGP'),
    "rule_24": CommandRule(r"^ip\\s+ip-prefix\\s+\\S+.*$", valid_modes=('global',), description='VRP prefix list'),
}
FORBIDDEN_PATTERNS = []
