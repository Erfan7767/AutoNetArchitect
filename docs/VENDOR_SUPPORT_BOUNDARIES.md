...
The default policy data intentionally contains empty supported-version prefixes and a `verification_required` status for every vendor family. This is a safe boundary: an administrator must load a reviewed, traceable policy for an exact platform/version path before configuration support can move beyond review.
