...
def test_cisco_wlc_templates_all_registered_templates_parse():
    registry = TemplateRegistry.from_json(Path("data/template_registry.json"))
    validator = TemplateValidator(registry)
    records = [record for record in registry.all() if record.template_id.startswith("cisco_wlc.")]
    assert records
...
def test_cisco_wlc_templates_metadata_has_guard_and_evidence_for_every_template():
    registry = TemplateRegistry.from_json(Path("data/template_registry.json"))
    records = [record for record in registry.all() if record.template_id.startswith("cisco_wlc.")]
