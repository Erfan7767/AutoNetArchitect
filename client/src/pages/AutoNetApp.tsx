...
function ConfigsPage({ project }: { project: ProjectRecord }) {
  ...
  const [draft, setDraft] = useState({ vendor: "", deviceName: "", artifactSummary: "", artifactPreview: "", featureGuard: "unknown" as const, unsupportedFeatureLog: "" });
  const submit = (event: React.FormEvent<HTMLFormElement>) => { event.preventDefault(); addMutation.mutate({ projectId: project.id, ...draft }, { onSuccess: () => setDraft({ vendor: "", deviceName: "", artifactSummary: "", artifactPreview: "", featureGuard: "unknown", unsupportedFeatureLog: "" }) }); };
...
function OperationsPage() {
  const signals: Array<{ title: string; detail: string; icon: LucideIcon; tone: "amber" | "slate" }> = [
    { title: "Drift detection", detail: "No data received", icon: Radar, tone: "amber" },
    { title: "Health checks", detail: "No check recorded", icon: Activity, tone: "slate" },
    { title: "Backup posture", detail: "No backup evidence", icon: CloudCog, tone: "amber" },
  ];
  return <div className="space-y-7"><TitleBlock eyebrow="07 · Operations" title="Operational reality" description="Read-only operational views compare evidence with the declared source of truth. No high-risk automated remediation is available through this interface." /><div className="grid gap-5 md:grid-cols-3">{signals.map(signal => { const Icon = signal.icon; return <section key={signal.title} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><div className="flex items-center justify-between"><div className="grid h-11 w-11 place-items-center rounded-xl bg-slate-50 text-slate-700"><Icon className="h-5 w-5" /></div><Pill tone={signal.tone}>Evidence needed</Pill></div><h2 className="mt-6 font-display text-xl font-semibold text-slate-950">{signal.title}</h2><p className="mt-2 text-sm leading-6 text-slate-500">{signal.detail}</p></section>; })}</div></div>;
}
